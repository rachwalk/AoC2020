def getNeighbourCount(x,y,z,w,space):
    count = 0
    for v in range(w-1, w+2):
        for a in range(z-1,z+2):
            for b in range(y-1, y+2):
                for c in range(x-1,x+2):
                    if not (a == z and b==y and c == x and v == w):
                        if (c,b,a,v) in space.keys():
                            count = count + 1
    return count
...
def evolveGoL(space):
    minw = min([x[3] for x in list(space.keys())])
    minz = min([x[2] for x in list(space.keys())])
    miny = min([x[1] for x in list(space.keys())])
    minx = min([x[0] for x in list(space.keys())])
    maxw = max([x[3] for x in list(space.keys())])
    maxz = max([x[2] for x in list(space.keys())])
    maxy = max([x[1] for x in list(space.keys())])
    maxx = max([x[0] for x in list(space.keys())])
    new_space = {}
    for v in range(minw-1, maxw+2):
        for i in range(minz-1, maxz+2):
            for j in range(miny-1, maxy+2):
                for k in range(minx-1, maxx+2):
                    count = getNeighbourCount(k,j,i,v,space)
                    if (k,j,i,v) in space.keys():
